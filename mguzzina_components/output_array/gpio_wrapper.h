...
class GPIOWrapper : public GPIOPin {
 public:
  ...
  bool digital_read() override {
    ...
    return this->pins_[this->current_pin_]->digital_read();
  }

  void digital_write(bool value) override {
    assert(!this->pins_.empty());
    std::lock_guard lock(this->pin_mutex_);
    if (this->current_pin_ == 0) {
    ...
    size_t activate_pin = this->current_pin_;
    if (!value) {
      --activate_pin;
    }
    size_t i = 0;
    for (; i < activate_pin; ++i) {
      this->pins_[i]->digital_write(false);
    }
    this->pins_[i++]->digital_write(true);
    for (; i < this->pins_.size(); ++i) {
      this->pins_[i]->digital_write(false);
    }
  }
  ...
  void enable_pin(size_t pin) {
    assert(!this->pins_.empty());
    std::lock_guard lock(this->pin_mutex_);
    this->current_pin_ = std::clamp(pin, 0U, this->pins_.size() - 1);
  }
